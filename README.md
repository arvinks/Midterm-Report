...
\usepackage{balance}
\begin{document}
\title{Lensless Imaging}
\author{Arvin Shahraki}
...
\section{Methods}
\subsection{PhlatCam: Learning the Ropes}
...
\subsubsection{2-Dimensional Reconstruction}
\noindent Once the phase map is designed, produced, and the camera is assembled and image can be captured and reconstructed. Because the scene depth is much larger than the thickness of the device, the reconstruction is done through Tinkhonov regularization with Wiener deconvolution \cite{phlatcam}. 
\begin{equation}
    \label{eq4}
...
\subsection{Reproducing the Phase Mask Design}
